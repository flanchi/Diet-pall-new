...
export default function AIChat({ user, medicalProfile, initialQuery, settings, mode = "full", minimized = false, unreadCount = 0, onToggleMinimize, onAiMessage }) {
  ...
  return (
    <div className={`neu-surface flex flex-col overflow-hidden ${isWidget ? "h-full rounded-2xl" : "h-[72vh] min-h-[420px] sm:min-h-[600px] rounded-lg"}`}>
      ...
      {/* Status bar */}
      {!showCompactWidget && (
        <div className={`p-3 neu-surface-inset text-sm ${isWidget ? "mb-3" : "mb-4"}`}>
        {user ? (
          <>
      ...
      {/* Chat messages */}
      {!showCompactWidget && (
        <div className={`flex-1 overflow-y-auto neu-surface-inset rounded p-4 space-y-3 min-h-0 ${isWidget ? "mb-3" : "mb-4"}`}>
        {messages.map(msg => (
          <div
            key={msg.id}
      ...
      {!showCompactWidget && (
        ...
        <input
          ...
          className="flex-1 px-4 py-2 neu-surface-inset focus:outline-none focus:ring-2 focus:ring-primary"
          disabled={loading}
        />
        <button
          onClick={handleSend}
