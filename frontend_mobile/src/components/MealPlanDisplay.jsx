...
export default function MealPlanDisplay({ mealPlan, onGetRecipe }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="neu-surface rounded-3xl p-4 sm:p-6 lg:p-8">
        <div className="space-y-2 mb-5 sm:mb-8">
          <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold gradient-text">Your Personalized Meal Plan</h3>
          <p className="text-slate-600 text-sm sm:text-lg">Optimized for your health conditions and allergies</p>
        </div>
        ...
        <div className="grid gap-6">
          {mealPlan.meals.map((meal, idx) => (
            <div
              key={idx}
              className={`group relative rounded-2.5xl p-4 sm:p-6 lg:p-8 border-2 bg-slate-100/85 ${mealGradients[meal.slot] || "border-slate-300"} card-hover overflow-hidden shadow-sm`}
            >
              ...
              <div className="relative space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div className="text-3xl sm:text-5xl">{mealEmojis[meal.slot] || ""}</div>
                    <div>
                      <h4 className="text-xl sm:text-2xl font-bold text-slate-900 capitalize">{meal.slot}</h4>
                ...
                <div className="pb-3 sm:pb-4 border-b-2 border-white/40">
                  <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 break-words">{meal.name}</p>
                </div>
                ...
                {/* Action buttons */}
                <div className="flex gap-3 pt-4 sm:pt-6 flex-col sm:flex-row">
                  <button
                    className="flex-1 bg-gradient-to-r from-primary-600 to-primary-500 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-full font-semibold text-xs sm:text-sm uppercase tracking-wide hover:shadow-glow-primary transition-all duration-300 transform hover:scale-105"
                    onClick={() => {
                      try {
                  ...
                  <button
                    className="flex-1 bg-gradient-to-r from-secondary-600 to-secondary-500 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-full font-semibold text-xs sm:text-sm uppercase tracking-wide hover:shadow-glow-secondary transition-all duration-300 transform hover:scale-105"
                    onClick={() => {
                      if (onGetRecipe) {
      ...
      {/* Footer info box */}
      <div className="neu-surface rounded-2.5xl p-6 border-l-4 border-accent-500 space-y-2 animate-slide-up">
